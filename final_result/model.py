...
def predict_integrated(path):
    # 品牌识别：EfficientNet-B0，224×224，[0,255] 输入（模型内置 Rescaling）
    ...
    # 种类识别：修正为 224×224，归一化到 [0,1]（与新版 train.py 一致）
    # 原代码用 128×128 且归一化到 [-1,1]，与新模型不匹配
    ...
    # AI 检测：128×128，不做归一化（模型内置 Rescaling 层，自己处理归一化）
    img_a  = image.load_img(path, target_size=(128, 128))
    x_a    = image.img_to_array(img_a)          # 保持原始像素值 [0, 255]，不要在外部归一化
    ...
    # 成色鉴定：EfficientNetB0 迁移学习版本，224×224，不做归一化
    # （EfficientNet 内置 Normalization 层，期望原始 [0, 255] 输入）
    # 输出 sigmoid ∈ [0,1]：越接近 1 越像 good_shoes（按字母序 dirty=0, good=1）
...
def analyze_integrity(text, m_type, m_score, m_aigc_text, m_brand, price_range):
    ...
    # A. 真伪验证（不符合扣40分）
    real_kws = r"实拍|真实|真图|原图|无修|非AI|非ai|不是AI|不是ai|无滤镜|实物|现货|未经修改|未经处理|纯实拍|纯原图|手机实拍|相机实拍|现场实拍|实物拍摄|现货拍摄|真实拍摄|原图拍摄|实拍原图|实拍真图|实拍无修|原图无修|真实无修|实物无修"
    ai_kws   = r"(?<!不是)(?<!非)(?<!不是 )(AI生成|ai生成|合成图|效果图|非实物图|虚拟渲染|AI作图|ai作图|AI绘图|ai绘图|AI合成|ai合成|算法生成|虚拟生成|智能作图|数字生成|人工合成|模型生成|机器生成|AI渲染|算法渲染|非实拍|非原图|非真实|非实物|虚拟图像|数字图像|算法图像|AI图像|ai图像|生成图像|伪造图像|AI制作|ai制作|算法制作|虚拟制作|AI创作|ai创作)"
    ...
    if m_brand in brand_reg and re.search(brand_reg[m_brand], text, re.I):
        details.append("<span style='color:green;'>品牌诚实 (不扣分)</span>")
    else:
        reason = "品牌错误" if any(re.search(r, text, re.I) for r in brand_reg.values()) else "品牌未提"
        score = max(0, score - 40)
        details.append(f"<span style='color:red;'>{reason} (-40分)</span>")
    ...
    type_reg = {
        ...
        "凉鞋":     r"凉鞋|沙滩凉鞋|露趾凉鞋|包趾凉鞋|罗马凉鞋|运动凉鞋|高跟凉鞋|平底凉鞋",
    ...
    if m_type in type_reg and re.search(type_reg[m_type], text):
        details.append("<span style='color:green;'>种类诚实 (不扣分)</span>")
    else:
        reason = "种类错误" if any(re.search(r, text) for r in type_reg.values()) else "种类未提"
        score = max(0, score - 40)
        details.append(f"<span style='color:red;'>{reason} (-40分)</span>")
    ...
    lv5_kws = r"全新|100%新|全新未穿|全新未用|全新闲置|全新收藏|全新保存|全新正品|全新未拆|崭新|崭新如初|完美|完美无瑕|无瑕|极品|零磨损|零瑕疵|零划痕|零破损|零使用痕迹|无任何瑕疵|未上脚|未穿着|未使用|全新未开封|全新无磨损|全新无瑕疵|绝对全新|绝对完美|真正全新|纯全新|全新到极致|完美到极致|几乎全新|九成九五新|九五新|九成八新|接近全新|近乎全新|准全新|基本全新|几乎未穿|几乎未用|极轻微磨损|极轻微痕迹|几乎无磨损|几乎无瑕疵|外观如新|内里如新|鞋底如新|无明显磨损|无明显瑕疵|品相极佳|成色极佳|状态极佳|保养极佳|仅试穿未出门|试穿一次|几乎未上脚|近乎全新状态|接近全新状态"
    ...
    lv3_kws = r"尚可|七成新|六成新|品相尚可|成色尚可|状态尚可|成色中等|品相中等|状态中等|轻微开胶|轻微脱线|轻微变形|轻微脱色|保养尚可|外观尚可|整体尚可|略有破损中度磨损|边角磨损|边缘磨损|鞋内磨损|鞋跟磨损|小面积污渍|小面积划痕|小面积破损|成色偏旧|品相偏旧|状态偏旧|整体偏旧|外观偏旧"
    ...
    lv1_kws = r"较差|四成新|三成新|二成新|一成新|品相较差|成色较差|状态较差|严重磨损|重度磨损|深度磨损|大面积磨损|严重瑕疵|大面积划痕|大面积污渍|顽固污渍|严重开胶|完全开胶|严重开裂|完全开裂|脱线|断底|断面|断帮|脱胶|掉底|掉面|掉帮|破洞|大面积破洞|扭曲变形|严重老化|脱漆|掉漆|陈旧|老旧|破旧|老化严重|磨损严重|瑕疵严重|破损严重"

    user_lv = -1
    if re.search(lv5_kws, text):   user_lv = 5
    elif re.search(lv4_kws, text): user_lv = 4
    elif re.search(lv3_kws, text): user_lv = 3
    elif re.search(lv2_kws, text): user_lv = 2
    elif re.search(lv1_kws, text): user_lv = 1
    ...
    if price_match:
        ...
        lower_bound = max(1.0, p_low * 0.9)
        upper_bound = max(1.0, p_high * 1.1)
        if lower_bound <= u_p <= upper_bound:
            details.append("<span style='color:green;'>价格诚实 (不扣分)</span>")
        else:
            if u_p < lower_bound:
                diff_ratio = (lower_bound - u_p) / lower_bound
                reason = "价格偏低"
            else:
                diff_ratio = (u_p - upper_bound) / upper_bound
                reason = "价格偏高"
    ...
    summary = f"<b style='color:{risk_color}; font-size:18px;'>{risk_text}</b> ({score}分)<br>"
    return score, summary + "<div style='font-size:11px; margin-top:5px;'>" + " | ".join(details) + "</div>"
...
def get_price_info(brand, s_type, h_score, ai_prob):
    ...
    if ai_prob >= 0.95:
       ...
       s = (0.95 - ai_prob) / 0.9          # s ∈ (0, 1)，p越低s越大
       m_ai = round(1 - 0.99 * (s ** 3), 4)  # 慢降→急降的三次方指数曲线
